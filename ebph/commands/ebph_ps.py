...
def format_comm(comm):
    return comm if len(comm) < 24 else ''.join(['...', comm[-21:]])

def print_profile_information(profile):
    comm = format_comm(profile["exe"])
    status = profile['status']
    status = status.split('EBPH_PROFILE_STATUS.')[1].capitalize()
    ...
    global header
    if not header:
        print(f"{'COMM':<24} {'STATUS':<20} {'TRAIN_COUNT':>12} {'LAST_MOD':>12} {'ANOMALIES':>12} {'NORMAL TIME':<16}")
        header = True

    print(f"{comm:<24} {status:<20} {train_count:>12} {last_mod_count:>12} "
