...
BPF_ARRAY(_ebph_settings, u64, EBPH_SETTING__END, 0);

static __always_inline u64 ebph_get_setting(int key)
{
    u64 *res = bpf_map_lookup_elem(&_ebph_settings, &key);
    if (!res) {
        return 0;
    }
...
{
    return bpf_map_update_elem(&_ebph_settings, &key, &val, 0);
}
...
BPF_HASH(task_states, u32, struct ebph_task_state_t, EBPH_MAX_PROCESSES, 0);

/* Profile Key -> Profile */
BPF_HASH(profiles, u64, struct ebph_profile_t, EBPH_MAX_PROFILES, 0);

/* Profile Key -> Syscall Flags */
BPF_HASH(training_data, u64, struct ebph_flags_t, EBPH_MAX_PROFILES,
         BPF_F_NO_PREALLOC);

/* Profile Key -> {Syscall Flags} */
BPF_HASH(testing_data, u64, struct ebph_flags_t, EBPH_MAX_PROFILES,
         BPF_F_NO_PREALLOC);

/* {PID (Kernel), Stack Top} -> Sequence Stack */
BPF_HASH(sequences, struct ebph_sequence_key_t, struct ebph_sequence_t,
         (EBPH_MAX_PROCESSES * EBPH_SEQSTACK_FRAMES), BPF_F_NO_PREALLOC);

/* PID (Kernel) -> Locality Frame */
BPF_HASH(locality_frames, u32, struct ebph_alf_t, EBPH_MAX_PROCESSES,
         BPF_F_NO_PREALLOC);

/* The following arrays are read-only, used for initializing large data. */

BPF_ARRAY(_init_flags, struct ebph_flags_t, 1, 0);
static __always_inline struct ebph_flags_t *ebph_new_flags()
{
    int zero = 0;
    return bpf_map_lookup_elem(&_init_flags, &zero);
}

BPF_ARRAY(_init_alf, struct ebph_alf_t, 1, 0);
static __always_inline struct ebph_alf_t *ebph_new_alf()
{
    int zero = 0;
    return bpf_map_lookup_elem(&_init_alf, &zero);
}
...
BPF_RINGBUF(new_profile_events, 4);
static __always_inline void ebph_log_new_profile(u64 profile_key,
                                                 const char *pathname)
{
    struct ebph_new_profile_event_t *event = bpf_ringbuf_reserve(
        &new_profile_events, sizeof(struct ebph_new_profile_event_t), 0);
    if (event) {
...
BPF_RINGBUF(anomaly_events, 3);
static __always_inline void ebph_log_anomaly(u16 syscall, int misses,
                                             struct ebph_task_state_t *s)
{
    struct ebph_anomaly_event_t *event = bpf_ringbuf_reserve(
        &anomaly_events, sizeof(struct ebph_anomaly_event_t), 0);
    if (event) {
...
BPF_RINGBUF(new_sequence_events, 3);
...
{
    struct ebph_new_sequence_event_t *event = bpf_ringbuf_reserve(
        &new_sequence_events, sizeof(struct ebph_new_sequence_event_t), 0);
    if (event) {
        event->pid           = s->pid;
        event->profile_key   = s->profile_key;
...
BPF_RINGBUF(start_normal_events, 3);
static __always_inline void ebph_log_start_normal(u64 profile_key,
                                                  struct ebph_task_state_t *s,
                                                  struct ebph_profile_t *p)
{
    struct ebph_start_normal_event_t *event = bpf_ringbuf_reserve(
        &start_normal_events, sizeof(struct ebph_start_normal_event_t), 0);
    if (event) {
        if (s) {
            event->pid        = s->pid;
            event->task_count = s->count;
...
BPF_RINGBUF(stop_normal_events, 3);
static __always_inline void ebph_log_stop_normal(u64 profile_key,
                                                 struct ebph_task_state_t *s,
                                                 struct ebph_profile_t *p)
{
    struct ebph_stop_normal_event_t *event = bpf_ringbuf_reserve(
        &stop_normal_events, sizeof(struct ebph_start_normal_event_t), 0);
    if (event) {
        if (s) {
            event->pid        = s->pid;
            event->task_count = s->count;
...
BPF_RINGBUF(tolerize_limit_events, 3);
static __always_inline void ebph_log_tolerize_limit(struct ebph_task_state_t *s,
                                                    struct ebph_alf_t *a)
{
    struct ebph_tolerize_limit_event_t *event = bpf_ringbuf_reserve(
        &tolerize_limit_events, sizeof(struct ebph_tolerize_limit_event_t), 0);
    if (event) {
        event->profile_key = s->profile_key;
        event->pid         = s->pid;
                                               ...
                                               u32 tgid, const char *pathname)
{
    ...
    bool profile_exists = bpf_map_lookup_elem(&profiles, &profile_key) ? 1 : 0;
                                              ...
                                              unsigned int tolerance_threshold)
{
    ...
    // Look up task state
    struct ebph_task_state_t *s = bpf_map_lookup_elem(&task_states, &pid);
    if (!s) {
        return 0;
    }

    // Look up profile
    struct ebph_profile_t *p = bpf_map_lookup_elem(&profiles, &s->profile_key);
    if (!p) {
        // TODO log error
        return 0;
    }
    ...
    struct ebph_alf_t *alf = bpf_map_lookup_elem(&locality_frames, &s->pid);
    if (!alf) {
        // TODO log error
        return 0;
    }
...
int BPF_PROG(task_free, struct task_struct *task)
{
    ...
    bpf_map_delete_elem(&locality_frames, &pid);
    ...
    for (int i = 0; i < EBPH_SEQSTACK_FRAMES; i++) {
        key.seqstack_top = i;
        bpf_map_delete_elem(&sequences, &key);
    }
...
int BPF_PROG(task_getscheduler, int unused)
{
    // TODO probably need to treat this as a special case
    // to reduce non-determinism
...
int BPF_PROG(unix_may_send, int unused)
{
    // TODO consider moving this to a common socket id
...
SEC("lsm/key_free")
int BPF_PROG(key_free, int unused)
{
...
int BPF_PROG(ptrace_access_check, int unused)
{
...
SEC("lsm/capget")
int BPF_PROG(capget, int unused)
{
...
SEC("lsm/capset")
int BPF_PROG(capset, int unused)
{
    // TODO: maybe split this by capabilities
...
SEC("lsm/capable")
int BPF_PROG(capable, int unused)
{
    // TODO: maybe split this by capabilities
...
int BPF_PROG(vm_enough_memory, int unused)
{
    return ebph_do_lsm_common(EBPH_VM_ENOUGH_MEMORY, EBPH_TOLERANCE_HIGH);
}

SEC("lsm/bpf")
int BPF_PROG(bpf, int unused)
{
...
SEC("lsm/bpf_map")
int BPF_PROG(bpf_map, int unused)
{
    // TODO: treat ebpH as a special case
...
SEC("lsm/bpf_prog")
int BPF_PROG(bpf_prog, int unused)
{
    // TODO: treat ebpH as a special case
...
int BPF_PROG(perf_event_open, int unused)
{
    // TODO: treat ebpH as a special case
...
int sys_exit_rt_sigreturn(struct trace_event_raw_sys_exit *args)
{
    u32 pid = bpf_get_current_pid_tgid();

    struct ebph_task_state_t *task_state =
        bpf_map_lookup_elem(&task_states, &pid);
    if (!task_state) {
        return 0;
    }
...
int do_signal_deliver(struct trace_event_raw_signal_deliver *args)
{
    bool monitoring = ebph_get_setting(EBPH_SETTING_MONITORING);
    if (!monitoring) {
        return 0;
...
//        bpf_map_lookup_elem(&profiles, &profile_key);
//    if (!profile) {
//        rc = -ESRCH;
//        goto out;
//    }
...
//
//    struct ebph_profile_t *profile =
//        bpf_map_lookup_elem(&profiles, &s->profile_key);
//    if (!profile) {
//        rc = -ESRCH;
//        goto out;
//    }
...
{
    // TODO set EBPH_BOOT_EPOCH using a global variable?
                                                 ...
                                                 u16 prev)
{
    ...
    flags = bpf_map_lookup_or_try_init(&training_data, &profile_key, flags);
    if (!flags) {
        // TODO log error
        return 0;
    }
                                                ...
                                                u16 prev)
{
    ...
    flags = bpf_map_lookup_or_try_init(&testing_data, &profile_key, flags);
    if (!flags) {
        // TODO log error
        return 0;
    }
...
{
    bpf_map_delete_elem(&training_data, &profile_key);
    p->anomaly_count  = 0;
    p->train_count    = 0;
    p->last_mod_count = 0;
}
    ...
    u32 pid, u32 tgid, u64 profile_key)
{
    ...
    return bpf_map_lookup_or_try_init(&task_states, &pid, &task_state);
}
...
static __always_inline int ebph_reset_alf(struct ebph_task_state_t *s)
{
    ...
    return bpf_map_update_elem(&locality_frames, &s->pid, alf, 0);
}
...
{
    struct ebph_profile_t *existing_profile =
        bpf_map_lookup_elem(&profiles, &profile_key);
    if (existing_profile)
    ...
    return bpf_map_lookup_or_try_init(&profiles, &profile_key, &profile);
}
    ...
    struct ebph_task_state_t *task_state)
{
    ...
    bpf_map_update_elem(&sequences, &key, &new_seq, 0);

    return bpf_map_lookup_elem(&sequences, &key);
}
...
{
    struct ebph_flags_t *training_flags =
        bpf_map_lookup_elem(&training_data, &profile_key);

    if (!training_flags) {
        return;
    }

    bpf_map_update_elem(&testing_data, &profile_key, &training_flags, 0);
}
